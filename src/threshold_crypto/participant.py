...
def compute_partial_re_encryption_key(
    old_share: KeyShare,
    old_lc: LagrangeCoefficient,
    new_share: KeyShare,
    new_lc: LagrangeCoefficient,
) -> PartialReEncryptionKey:
    """
    ...
    partial_re_key = (
        new_share.y * new_lc.coefficient - old_share.y * old_lc.coefficient
    ) % curve_params.order
...
class Participant:
    ...
    def __init__(
        ...
        if len(set(all_participant_ids)) != threshold_params.n:
            raise ThresholdCryptoError(
                "List of distinct participant ids has length {} != {} = n".format(
                    len(all_participant_ids), threshold_params.n
                )
            )
        ...
        self._received_closed_commitments: Dict[ParticipantId, DkgClosedCommitment] = {
            self.id: self.closed_commitment()
        }
    ...
    def closed_commitment(self) -> DkgClosedCommitment:
        """
    ...
    def receive_closed_commitment(self, commitment: DkgClosedCommitment):
        ...
        if source_id not in self.all_participant_ids:
            raise ThresholdCryptoError(
                "Received closed commitment from unknown participant id {}".format(
                    source_id
                )
            )
        ...
        if source_id not in self._received_closed_commitments:
            ...
            raise ThresholdCryptoError(
                "Closed commitment from participant {} already received".format(
                    source_id
                )
            )
    ...
    def _unchecked_open_commitment(self) -> DkgOpenCommitment:
        ...
        return DkgOpenCommitment(
            self.id, self._commitment, self._h_i, self._commitment_random
        )
    ...
    def receive_open_commitment(self, open_commitment: DkgOpenCommitment):
        ...
        if source_id not in self.all_participant_ids:
            raise ThresholdCryptoError(
                "Received open commitment from unknown participant id {}".format(
                    source_id
                )
            )
        ...
        if source_id not in self._received_closed_commitments:
            raise ThresholdCryptoError(
                "Received open commitment from participant id {} withput received closed commitment".format(
                    source_id
                )
            )
        ...
        if closed_commitment.commitment != open_commitment.commitment:
            raise ThresholdCryptoError(
                "Open and close commitment values differ for participant {}".format(
                    source_id
                )
            )

        if (
            self._compute_commitment(open_commitment.r, open_commitment.h_i)
            != closed_commitment.commitment
        ):
            raise ThresholdCryptoError(
    ...
    def receive_F_ij_value(self, F_ij: DkgFijValue):
        ...
        if source_id not in self.all_participant_ids:
            raise ThresholdCryptoError(
                "Received F_ij values from unknown participant id {}".format(source_id)
            )
        ...
        if len_F_ij != self.threshold_params.t:
            raise ThresholdCryptoError(
                "List of F_ij values from participant {} has length {} != {} = t".format(
                    source_id, len_F_ij, self.threshold_params.t
                )
            )
    ...
    def s_ij_value_for_participant(
        self, target_participant_id: ParticipantId
    ) -> DkgSijValue:
        """
    ...
    def _unchecked_s_ij_value_for_participant(
        ...
        if target_participant_id not in self.all_participant_ids:
            raise ThresholdCryptoError(
                "Participant id {} not present in known participant ids".format(
                    target_participant_id
                )
            )
        else:
            return DkgSijValue(
                self.id, target_participant_id, self._local_sij[target_participant_id]
            )
    ...
    def receive_sij(self, received_sij: DkgSijValue):
        ...
        if source_id not in self.all_participant_ids:
            raise ThresholdCryptoError(
                "Received s_ij value from unknown participant id {}".format(source_id)
            )
        ...
        if target_id != self.id:
            raise ThresholdCryptoError(
                "Received s_ij value for foreign participant (own id={}, target id={})".format(
                    self.id, target_id
                )
            )
        ...
        if source_id not in self._received_sij:
            ...
            raise ThresholdCryptoError(
                "s_ij value for participant {} already received".format(source_id)
            )
    ...
    def compute_share(self) -> KeyShare:
        ...
        if len(self._received_sij) != self.threshold_params.n:
            raise ThresholdCryptoError(
                "Received less s_ij values than necessary: {} != {} = n".format(
                    len(self._received_sij), self.threshold_params.n
                )
            )

        self._s_i = (
            sum(rs.s_ij for rs in self._received_sij.values()) % self.curve_params.order
        )
    ...
    def __str__(self):
        return "Participant[id = {}, x_i = {}, h_i = {}, s_i = {}".format(
            self.id, self._x_i, self._h_i, self._s_i
        )
