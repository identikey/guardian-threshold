...
class TCTestCase(unittest.TestCase):
    ...
    def test_key_encryption_decryption_with_enough_shares(self):
        ...
        partial_decryptions = [
            participant.compute_partial_decryption(em, share)
    ...
    def test_key_encryption_decryption_without_enough_shares(self):
        ...
        em = EncryptedMessage(kP, c, b"")
        reconstruct_shares = [
            self.shares[i] for i in [0, 4]
        ]  # choose 2 of 5 key shares
        partial_decryptions = [
            participant.compute_partial_decryption(em, share)
            for share in reconstruct_shares
        ]
    ...
    def test_complete_process_with_enough_shares(self):
        ...
        pub_key, key_shares = central.create_public_key_and_shares_centralized(
        ...
        reconstruct_shares = [
            key_shares[i] for i in [0, 2, 4]
        ]  # choose 3 of 5 key shares
        partial_decryptions = [
            participant.compute_partial_decryption(encrypted_message, share)
            for share in reconstruct_shares
        ]
        decrypted_message = central.decrypt_message(
...
class PreTestCase(unittest.TestCase):
    ...
    def parameterizable_re_encryption_process_test(self, new_t: int, new_n: int):
        ...
        new_pk, new_shares = central.create_public_key_and_shares_centralized(
            self.cp, new_tp
        )

        assert (
            new_pk != self.pk
        ), "Public keys for new and old access structure are the same"
        ...
        for i, (s_old, s_new) in enumerate(zip(t_old_shares, t_new_shares)):
            old_lambda = central.lagrange_coefficient_for_key_share_indices(
                t_old_shares_x, t_old_shares_x[i], self.cp
            )
            new_lambda = central.lagrange_coefficient_for_key_share_indices(
                t_new_shares_x, t_new_shares_x[i], self.cp
            )
            partial_key = participant.compute_partial_re_encryption_key(
                s_old, old_lambda, s_new, new_lambda
            )
            partial_re_encrypt_keys.append(partial_key)

        re_encrypt_key = central.combine_partial_re_encryption_keys(
            partial_re_encrypt_keys, self.pk, new_pk, self.tp, new_tp
        )
        ...
        new_partial_decryptions = [
            participant.compute_partial_decryption(re_em, share)
            for share in new_reconstruct_shares
        ]

        decrypted_message = central.decrypt_message(
            new_partial_decryptions, re_em, new_tp
        )
        ...
        with self.assertRaises(ThresholdCryptoError) as dec_exception_context:
            ...
            new_partial_decryptions = [
                participant.compute_partial_decryption(re_em, share)
                for share in less_reconstruct_shares
            ]
...
class DkgTestCase(unittest.TestCase):
    ...
    def test_open_commitment_json(self):
        c = DkgOpenCommitment(
            1, random.getrandbits(10), self.cp.P, random.getrandbits(10)
        )
    ...
    def test_distributed_key_generation(self):
        ...
        participants = [
            participant.Participant(id, participant_ids, self.cp, self.tp)
        ...
        pdms = [
            participant.compute_partial_decryption(em, ks) for ks in shares[: self.tp.t]
        ]
